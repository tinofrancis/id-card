...
import { ShieldCheck, Compass, UserCheck, ArrowRight, Loader2 } from 'lucide-react';
...
export default function VerifyClient({
  ...
  const activeThemeId = profileData?.theme || initialTheme;
  const activeTheme = THEMES.find((t) => t.id === activeThemeId) || THEMES[0];

  const displayName = profileData?.name || initialName;
  const displayRole = profileData?.role || initialRole;
  const displayTitle = profileData?.title || initialTitle;
